...
if __name__ == '__main__':
    ...
    tf.app.flags.DEFINE_string("val_root", "/home/rpng/datasets/car_masks/", "")
...
def writeFileList_custom(dirName):
    im_list = [] # list of all files with full path
    lab_list = [] # list of all files with full path
    for dirname, dirnames, filenames in os.walk(dirName):
        for filename in filenames:
            if filename.endswith('.jpg'):	
                ...
                bnl = fileName.split('.jpg')
                lab_list.append(bnl[0] + "_color_mask.png")
...
def generate():
    ...
    mean = np.zeros((3))
    for k in range(2):
        car_id = 26 if k==0 else 16
    
        if k==0:
            im_list, lab_list = writeFileList(FLAGS.cityscapes_root)
        else:
            im_list, lab_list = writeFileList_custom(FLAGS.val_root)
